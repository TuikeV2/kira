...
export default function BuyLicense() {
  ...
  useEffect(() => {
    dashboardService.getPublicStats()
      .then(res => {
        ...
        if (d) {
          setHeroStats([
            { value: `${d.totalServers}+`, label: 'Serwerów', icon: FaServer },
            { value: d.totalUsers >= 1000 ? `${Math.round(d.totalUsers / 1000)}K+` : `${d.totalUsers}+`, label: 'Użytkowników', icon: FaUsers },
            { value: `${d.uptime}%`, label: 'Uptime', icon: FaBolt },
            { value: '24/7', label: 'Wsparcie', icon: FaHeadset }
          ]);
        }
      })
        ...
        <motion.div variants={fadeUp}>
          {verifyingPayment ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-gray-400">Pobieranie klucza licencji...</span>
            </div>
          ) : purchasedLicense?.licenseKey ? (
            <GlassCard className="p-8 mb-8 border-2 border-amber-500/20 bg-amber-500/10">
              <div className="flex items-center justify-center gap-2 mb-4">
                <FaKey className="w-6 h-6 text-amber-500" />
                <span className="font-bold text-lg text-white">Twój klucz licencji</span>
              </div>
              <div className="flex items-center gap-3 mb-6">
                <code className="flex-1 bg-white/5 border-2 border-amber-500/20 px-6 py-5 rounded-xl font-mono text-xl text-center text-white font-bold tracking-widest">
                  {purchasedLicense.licenseKey}
                </code>
                <button
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="bg-white/5 rounded-lg p-3">
                  ...
                  <span className="ml-2 font-bold text-purple-400">{purchasedLicense.tier}</span>
                </div>
                <div className="bg-white/5 rounded-lg p-3">
                  <span className="text-gray-400">Wygasa:</span>
                  <span className="ml-2 font-bold text-white">{formatDate(purchasedLicense.expiresAt)}</span>
                </div>
              </div>
            </GlassCard>
          ) : (
            <GlassCard className="p-6 mb-8 border-red-500/20 bg-red-500/10">
              <p className="text-red-400">Nie udało się pobrać klucza. Sprawdź zakładkę "Moje Licencje".</p>
            </GlassCard>
          )}
        </motion.div>
  ...
  if (paymentStatus === 'canceled') {
    return (
      ...
      >
        <motion.div variants={fadeUp}>
          <div className="w-20 h-20 rounded-full bg-red-500/15 ring-1 ring-red-500/20 flex items-center justify-center mx-auto mb-6">
            <FaTimesCircle className="w-10 h-10 text-red-400" />
          </div>
        </motion.div>
        <motion.div variants={fadeUp}>
          <h1 className="text-3xl font-bold text-white mb-4">Płatność anulowana</h1>
    ...
    <motion.div className="space-y-12" variants={stagger} initial="hidden" animate="show">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <motion.div variants={fadeUp}>
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 rounded-xl bg-amber-500/15 text-amber-400 ring-1 ring-amber-500/20">
                <FaCrown className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-2xl font-bold text-white">Wybierz plan</h2>
                <p className="text-gray-400">Im dłuższy okres, tym więcej oszczędzasz</p>
              </div>
            </div>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        p.isPopular
                          ? 'bg-gradient-to-b from-primary-500 to-purple-600 border border-white/20'
                    ...
                    >
                      ...
                      <div className={`p-6 ${p.isPopular ? 'text-white' : ''}`}>
                        {/* Tier badge */}
                        <div className={cn(
                          'inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold mb-3',
                          p.isPopular ? 'bg-white/20' : 'bg-purple-500/15 text-purple-400 ring-1 ring-purple-500/20'
                        )}>
                          <FaGem className="w-3 h-3" />
                          {p.tier || 'VIP'}
                        </div>

                        <h3 className={`text-xl font-bold mb-2 ${p.isPopular ? 'text-white' : 'text-white'}`}>
                          {p.name}
                        </h3>

                        {p.description && (
                          <p className={`text-sm mb-4 ${p.isPopular ? 'text-white/80' : 'text-gray-400'}`}>
                            {p.description}
                          </p>
                        )}
                        ...
                        <div className="mb-4">
                          <div className="flex items-baseline gap-1">
                            ...
                            <span className={p.isPopular ? 'text-white/70' : 'text-gray-400'}>zł</span>
                          </div>
                          <p className={`text-sm ${p.isPopular ? 'text-white/70' : 'text-gray-400'}`}>
                        ...
                        {p.features && p.features.length > 0 && (
                          <div className="space-y-2 mb-4">
                            {p.features.slice(0, 4).map((feature, idx) => (
                              <div key={idx} className={`flex items-center gap-2 text-sm ${p.isPopular ? 'text-white/90' : 'text-gray-400'}`}>
          ...
          <motion.div variants={fadeUp}>
            <GlassCard className="p-6 relative">
              ...
              {!showAllFeatures && (
                <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-dark-950 to-transparent pointer-events-none rounded-b-2xl"></div>
              )}
            </GlassCard>
          </motion.div>
          ...
          <motion.div variants={fadeUp}>
            <GlassCard className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 rounded-xl bg-green-500/15 text-green-400 ring-1 ring-green-500/20">
                  <FaTag className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">Kod rabatowy</h3>
                  <p className="text-sm text-gray-400">Masz kod? Użyj go tutaj!</p>
                </div>
              </div>
              ...
              {promoDiscount ? (
                <div className="flex items-center justify-between p-4 bg-green-500/10 rounded-xl border border-green-500/20">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-green-300">Kod: {promoCode}</p>
                      <p className="text-sm text-green-400">
                        Rabat: {promoDiscount.type === 'percentage' ? `-${promoDiscount.value}%` : `-${promoDiscount.value} zł`}
                      </p>
                    </div>
                  </div>
                  <button onClick={handleRemovePromo} className="p-2 text-green-400 hover:bg-green-500/10 rounded-lg">
                    <FaTimes />
                  </button>
                </div>
              ) : (
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className={cn(
                      'flex-1 px-4 py-3 rounded-xl border bg-white/5 backdrop-blur font-mono uppercase text-white placeholder-gray-500',
                      promoError ? 'border-red-500/30' : 'border-white/10'
                    )}
                  />
                  <button
                    onClick={handleValidatePromo}
                    disabled={promoLoading || !promoCode.trim()}
                    className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-medium disabled:opacity-50 transition-colors flex-shrink-0"
                  >
              ...
              <GlassCard className="p-6 shadow-lg">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between items-center py-3 border-b border-white/5">
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-lg bg-amber-500/15 text-amber-400 ring-1 ring-amber-500/20">
                        <FaCrown className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="font-medium text-white">Licencja {plan?.tier || 'VIP'}</p>
                        <p className="text-sm text-gray-400">{plan?.name}</p>
                      </div>
                    </div>
                    <span className="font-bold text-white">{plan ? parseFloat(plan.price).toFixed(0) : 0} zł</span>
                  </div>

                  {discount > 0 && (
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-green-500/15 text-green-400 ring-1 ring-green-500/20">
                          <FaPercent className="w-4 h-4" />
                        </div>
                        <span className="text-green-400">Rabat</span>
                      </div>
                      <span className="font-bold text-green-400">-{discount.toFixed(2)} zł</span>
                    </div>
                  )}

                  <div className="flex justify-between items-center pt-2">
                    <span className="text-lg font-bold text-white">Do zapłaty</span>
                    <div className="text-right">
                      {discount > 0 && (
                        <span className="text-sm text-gray-500 line-through mr-2">{plan ? parseFloat(plan.price).toFixed(0) : 0} zł</span>
                      )}
                      <span className="text-3xl font-bold text-white">{finalPrice.toFixed(0)} zł</span>
                    </div>
                  </div>
                </div>
            ...
            <motion.div variants={fadeUp}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-green-300">Gwarancja satysfakcji</h4>
                </div>
            ...
            <motion.div variants={fadeUp}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h4 className="font-bold text-white">Potrzebujesz pomocy?</h4>
                </div>
